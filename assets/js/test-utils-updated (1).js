...
/** Resolve question/explanation image from common field names */
...
/** Make relative asset paths root-absolute so they work from any page depth */
...
function getRandomTest(questions, n = "all") {
  ...
  return shuffledQuestions.map((q) => {
    ...
    return {
      q: q.q,
      photo: resolveMediaUrl(pickQuestionPhoto(q)),
      img: resolveMediaUrl(pickExplanationImg(q)),
      a: shuffledAnswers,
...
function setExamSessionMode(mode, opts = {}) {
  ...
  updateExamHeader();
  // Study = no timer; Exam = timer runs
  syncTimerWithSessionMode();
...
/** Study mode: no time limit. Exam mode: countdown timer. */
function syncTimerWithSessionMode() {
  ...
  items.forEach((q, index) => {
    ...
    card.innerHTML = `
      <div class="d-flex flex-column gap-3">
        ...
        ${
          q.explanation || q.img
            ? `<div class="card border-0 bg-light"><div class="card-body"><div class="fw-semibold mb-2">Izoh</div>${
                q.explanation
                  ? `<div class="text-secondary">${q.explanation}</div>`
                  : ""
              }${
                q.img
                  ? `<div class="mcq-explanation-img mt-3"><img src="${q.img}" alt="Izoh rasmi" class="mcq-responsive-img img-fluid rounded"></div>`
                  : ""
              }</div></div>`
            : ""
        }
...
function ensureExamHeaderStyles() {
  ...
  style.textContent = `
    ...
    /* Responsive question + explanation images (photo + img) */
    .mcq-photo,
    .mcq-explanation-img {
      text-align: center;
...
function showQuestion(isNavigatingBack = false) {
  ...
  const questionImg =
    resolveMediaUrl(q.photo) || resolveMediaUrl(pickQuestionPhoto(q));
  const photoHtml = questionImg
    ? `<div class="mcq-photo"><img src="${questionImg}" alt="Savol rasmi" loading="eager" class="mcq-responsive-img"></div>`
    : "";
...
function renderCaseControls() {
  ...
  const q = currentTest[currentIndex];
  // Study = show izoh + img; Exam = hide both (strict)
  ...
  const explanationImgHtml = explanationImg
    ? `<div class="mcq-explanation-img"><img src="${explanationImg}" alt="Izoh rasmi" loading="eager" class="mcq-responsive-img"></div>`
    : "";
...
function startCaseTimer() {
  ...
  // Study mode: no time limit — do not start countdown
  if (!isExamStrictMode()) {
    const timerEl = document.getElementById("timer");
    if (timerEl) {
      timerEl.style.display = "none";
