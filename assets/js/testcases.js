...
/* ============================================================
   UWORLD-STYLE CASE UI
============================================================ */

function ensureCaseStyles() {
  if (document.getElementById("uworld-case-styles")) return;

  const style = document.createElement("style");
  style.id = "uworld-case-styles";
  style.textContent = `
    #test-screen{max-width:720px;margin:0 auto;padding:12px 14px 40px;font-family:Inter,system-ui,-apple-system,sans-serif}
    #test-title{font-size:.85rem;font-weight:600;color:#64748b;margin-bottom:12px;letter-spacing:.02em}
    .uw-question-card{background:#fff;border-radius:16px;border:1px solid #e2e8f0;box-shadow:0 4px 20px rgba(15,23,42,.06);padding:20px 18px;margin-bottom:14px}
    .uw-question-text{font-size:1.05rem;font-weight:600;line-height:1.55;color:#0f172a;margin:0 0 16px}
    .uw-photo-wrap{text-align:center;margin:12px 0 18px}
    .uw-photo-wrap img{max-width:100%;border-radius:12px;box-shadow:0 8px 24px rgba(15,23,42,.1)}
    .uw-answers{display:flex;flex-direction:column;gap:10px}
    .uw-answer-btn{display:flex;align-items:flex-start;gap:12px;width:100%;text-align:left;padding:14px 16px;border-radius:12px;border:1.5px solid #e2e8f0;background:#f8fafc;color:#0f172a;font-size:.95rem;font-weight:500;line-height:1.45;cursor:pointer;transition:all .18s ease;font-family:inherit}
    .uw-answer-btn:hover:not(:disabled){border-color:#93c5fd;background:#eff6ff}
    .uw-answer-btn.active{border-color:#2563eb;background:#eff6ff;box-shadow:0 0 0 3px rgba(37,99,235,.15)}
    .uw-answer-btn.btn-success{border-color:#16a34a!important;background:#ecfdf5!important;color:#166534!important}
    .uw-answer-btn.btn-danger{border-color:#dc2626!important;background:#fef2f2!important;color:#991b1b!important}
    .uw-answer-btn:disabled{cursor:default;opacity:1}
    .uw-answer-btn i{flex-shrink:0;margin-top:2px;font-size:1.1rem}
    .uw-controls{margin-top:16px}
    .uw-check-btn,.uw-next-btn{width:100%;padding:14px 18px;border:none;border-radius:12px;font-size:.95rem;font-weight:600;font-family:inherit;cursor:pointer;transition:all .2s}
    .uw-check-btn{background:#2563eb;color:#fff}
    .uw-check-btn:disabled{background:#cbd5e1;cursor:not-allowed}
    .uw-check-btn:not(:disabled):hover{background:#1d4ed8}
    .uw-next-btn{background:#2563eb;color:#fff;margin-bottom:12px}
    .uw-next-btn:hover{background:#1d4ed8}
    .uw-explanation{margin-top:14px;background:#f0fdf4;border:1px solid #bbf7d0;border-left:4px solid #16a34a;border-radius:12px;padding:16px 18px}
    .uw-explanation-title{font-size:.8rem;font-weight:700;text-transform:uppercase;letter-spacing:.04em;color:#15803d;margin-bottom:8px;display:flex;align-items:center;gap:6px}
    .uw-explanation-body{font-size:.92rem;line-height:1.6;color:#334155;white-space:pre-wrap;word-break:break-word}
    #timer{font-variant-numeric:tabular-nums;font-weight:700;color:#2563eb}
    @media (max-width:640px){
      #test-screen{padding:10px 10px 32px}
      .uw-question-card{padding:16px 14px;border-radius:14px}
      .uw-question-text{font-size:1rem}
      .uw-answer-btn{padding:13px 14px;font-size:.9rem}
      .uw-explanation{padding:14px}
      .uw-explanation-body{font-size:.88rem}
    }
  `;
  document.head.appendChild(style);
}
...
function showQuestion() {
  ...
  if (titleEl) {
    titleEl.innerText = `Savol ${currentIndex + 1} / ${currentTest.length}`;
  }
  ...
  if (navbar) navbar.style.display = q.photo ? "none" : "";

  const photoHtml = q.photo
    ? `<div class="uw-photo-wrap"><img src="${q.photo}" class="img-fluid" alt="Savol rasmi" loading="eager"></div>`
    : "";

  container.innerHTML = `
    <div class="uw-question-card">
      <p class="uw-question-text">${q.q}</p>
      ${photoHtml}
      <div id="answer-buttons" class="uw-answers"></div>
      <div id="case-controls" class="uw-controls"></div>
    </div>`;
  ...
  q.a.forEach((ans, index) => {
    ...
    btn.type = "button";
    btn.className = "uw-answer-btn";
    btn.innerHTML = `<i class="bi bi-circle"></i><span>${ans}</span>`;
...
function renderCaseControls() {
  ...
  if (!answerChecked) {
    controls.innerHTML = `
      <button type="button" id="check-answer-btn" class="uw-check-btn" disabled>
        Tekshirish
      </button>`;
  ...
  controls.innerHTML = `
    <button type="button" id="next-question-btn" class="uw-next-btn">
      ${currentIndex + 1 >= currentTest.length ? "Natijani ko‘rish" : "Keyingi savol"}
    </button>
    ${
      hasExplanation
        ? `<div class="uw-explanation">
            <div class="uw-explanation-title"><i class="bi bi-lightbulb"></i> Izoh</div>
            <div class="uw-explanation-body">${q.explanation}</div>
          </div>`
        : ""
    }`;
