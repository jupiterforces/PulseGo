...
function showQuestion(isNavigatingBack = false) {
  ...
  const questionImg = q.photo || null;
...
window.getPulseTestDebug = function () {
  ...
  if (Array.isArray(bank) && q?.q) {
    const src = bank.find((x) => x && String(x.q).trim() === String(q.q).trim());
    if (src) {
...
Object.defineProperty(window, "currentTest", { get() { return currentTest; }, configurable: true });
Object.defineProperty(window, "currentIndex", { get() { return currentIndex; }, configurable: true });
Object.defineProperty(window, "answerChecked", { get() { return answerChecked; }, configurable: true });
