...
/* ===== Per-test attempt snapshots (review completed runs) ===== */
const ATTEMPT_SNAP_KEY = "pulsego_attempt_snapshots_v1";

function loadAttemptSnapshots() {
  try {
    const raw = localStorage.getItem(ATTEMPT_SNAP_KEY);
...
function saveAttemptSnapshot(testName, payload) {
  if (!testName) return;
  try {
    const all = loadAttemptSnapshots();
    all[testName] = {
      testName,
      questions: payload.questions || [],
      examQuestionResults: payload.examQuestionResults || {},
      examFlagged: payload.examFlagged || [],
      examNotes: payload.examNotes || {},
      score: payload.score || 0,
      total: payload.total || 0,
      date: new Date().toISOString(),
    };
    localStorage.setItem(ATTEMPT_SNAP_KEY, JSON.stringify(all));
  } catch (e) {
    console.error("snapshot save failed", e);
  }
}

function getAttemptSnapshot(testName) {
  const all = loadAttemptSnapshots();
  return all[testName] || null;
}

function getAllExamNotesFromStorage() {
  const notes = [];
  // live progress
  try {
    const p = loadTestProgress && null;
  } catch (e) {}
  try {
    const raw = localStorage.getItem(PROGRESS_KEY);
    if (raw) {
      const data = JSON.parse(raw);
      const map = data.examNotes || {};
      Object.keys(map).forEach((idx) => {
        const t = (map[idx] || "").trim();
        if (t) {
          notes.push({
  ...
  try {
    const all = loadAttemptSnapshots();
    Object.keys(all).forEach((tn) => {
      const snap = all[tn];
      const map = snap.examNotes || {};
      Object.keys(map).forEach((idx) => {
        const t = (map[idx] || "").trim();
        if (t) {
          notes.push({
...
window.getAttemptSnapshot = getAttemptSnapshot;
window.getAllExamNotesFromStorage = getAllExamNotesFromStorage;
window.saveAttemptSnapshot = saveAttemptSnapshot;
...
function normalizeSessionMode(raw) {
  ...
  if (["study", "study_mode", "study-mode", "o'rganish", "organish"].includes(m))
    return "study";
  return null;
}
...
function showResultPopup(options = {}) {
  ...
  // Snapshot for review mode later
  try {
    saveAttemptSnapshot(currentTestName || "unknown", {
...
async function startTest(testName) {
  ...
  // Progress dan davom ettirish (skip if forceNew)
  if (!forceNew) {
    ...
    if (
      ...
      (saved.currentIndex > 0 ||
        (saved.examQuestionResults &&
          Object.keys(saved.examQuestionResults).length > 0))
    ) {
      // Auto-resume when opened from listing "Davom" — only confirm if ambiguous
      const auto = options.autoResume === true;
      const resume = auto
        ? true
        : confirm(
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  examSessionMode = resolveSessionMode();
  document.body.classList.toggle("pulse-exam-strict", examSessionMode === "exam");
  document.body.classList.toggle("pulse-study-mode", examSessionMode === "study");
  initTestStats();
  initPrepContext();
  showPrepBanner();
