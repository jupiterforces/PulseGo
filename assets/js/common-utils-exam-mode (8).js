...
function cycleExamFontSize() {
  ...
  if (label) label.textContent = `${Math.round(examFontScale * 100)}%`;
}
...
function openExamSettings() {
  ...
  if (!modal) {
    ...
    document.getElementById("examFontDown").onclick = () => {
      const steps = [0.9, 1, 1.12, 1.25];
      const i = Math.max(0, steps.indexOf(examFontScale) - 1);
      examFontScale = steps[i];
      document.documentElement.style.setProperty("--exam-font-scale", String(examFontScale));
      document.getElementById("examFontScaleLabel").textContent = `${Math.round(examFontScale * 100)}%`;
    };
