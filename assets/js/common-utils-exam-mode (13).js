...
function ensureExamSelectionToolbar() {
  if (!document.getElementById("exam-sel-toolbar")) {
    ...
    tip.addEventListener("touchstart", (e) => e.preventDefault(), { passive: false });
...
function positionExamSelectionToolbar() {
  ...
  left = Math.max(8, Math.min(left, window.scrollX + window.innerWidth - tipW - 8));
...
async function startTest(testName) {
  ...
  if (
    ...
    if (resume) {
      ...
      examQuestionResults =
        saved.examQuestionResults && typeof saved.examQuestionResults === "object"
      ...
      if (examQuestionResults[currentIndex]) {
        answerChecked = true;
        selectedAnswerIndex = examQuestionResults[currentIndex].selected ?? null;
      }
    } else {
      clearTestProgress();
    }
...
function startWeakPath(testIds = null, maxQuestions = 15) {
  const ids = Array.isArray(testIds) && testIds.length
    ? testIds
  ...
  results.forEach((e) => {
    ...
    if (!byTest[name]) byTest[name] = { correct: 0, solved: 0, lastPercent: 100 };
    const solved = e.solved != null ? e.solved : (e.correct || 0) + (e.wrong || 0);
  ...
  const ranked = ids
    .map((name) => {
      ...
      const avg = st.solved ? Math.round((st.correct / st.solved) * 100) : st.lastPercent;
