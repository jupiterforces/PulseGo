...
function saveTestResult(resultData) {
  ...
  try {
    ...
    const solved = resultData.solved != null ? resultData.solved : correct + (resultData.wrong || 0);
    const wrong = resultData.wrong != null ? resultData.wrong : Math.max(0, solved - correct);
    const unsolved = resultData.unsolved != null ? resultData.unsolved : Math.max(0, total - solved);
    const percent =
...
function createExamBottomBar() {
  ...
  document.getElementById("exam-tool-settings").onclick = () => openExamSettings();
...
function applyExamFontScale() {
  ...
  document.documentElement.style.setProperty("--exam-font-scale", String(scale));
  const container = document.getElementById("question-container");
  if (container) {
    container.style.fontSize = `${scale}rem`;
    container.querySelectorAll(".mcq-question, .mcq-option, .mcq-option-text, .mcq-option-letter").forEach((el) => {
...
const EXAM_MOTIVATE_CORRECT = [
  "Juda zo'r, bolakay — kayfiyatlar zo'r-a!",
  "Barakalla, bo'tam!",
  "Zo'r ishlading — shunday davom et!",
  "Ajoyib! Biliming o'sib boryapti.",
  "To'g'ri! Sen buni uddalaysan.",
  "Qoyil, juda yaxshi javob!",
  "Mana shu — professional yondashuv!",
  "Yaxshi! Har bir to'g'ri javob — qadam oldinga.",
  "Barakalla! Diqqating joyida.",
  "Zo'r! Case'ni to'g'ri o'qiding.",
  "Davom et, champión!",
  "To'g'ri topding — ajoyib!",
  "Biliming mustahkam, shunday ket!",
  "Yaxshi ish! O'zingni maqtashing mumkin.",
  "Juda yaxshi! Keyingi savolga tayyormisan?",
  "Mukammal! Shunday tempda bor.",
  "Rahmat aql-zakovatga — to'g'ri!",
  "Bo'ldi, zo'r! Keling, keyingisini ham yechamiz.",
  "To'g'ri yo'ldasan, bolakay!",
  "Qoyilmaqom! Klinika ruhing bor.",
  "A'lo natija — g'ururlanishga arziydi!",
  "Ishonching oshsin: sen qila olasan!",
];

const EXAM_MOTIVATE_WRONG = [
  "Siqilma, bu kunlar ham o'tib ketadi.",
  "Xato — o'rganishning bir qismi. Davom et!",
  "Mayli, keyingi savolda o'zini ko'rsatasan.",
  "Hali erta, sen o'syapsan.",
  "Xatodan saboq ol — keyingisi bo'ladi.",
  "Tinchlan, chuqurroq o'yla, keyin oldinga.",
  "Hamma xato qiladi — muhimi tuzatish.",
  "Bu ham o'tadi. Keyingi savolga diqqat!",
  "Yiqilish — turish uchun. Turib ol!",
  "Bilim shu yo'l bilan mustahkamlanadi.",
  "Xato bo'ldi, lekin sen yo'ldasan.",
  "Qayta o'rganamiz — hech narsa yo'qolmaydi.",
  "Sabr qil, natija keladi.",
  "Bugungi xato — ertangi to'g'ri javob.",
  "O'zingni ayblama: tahlil qil va oldinga.",
  "Klinika shu bilan o'rganiladi — davom et.",
  "Yaxshi urinish edi, keyingisida zo'r bo'lasan.",
  "Bu savolni keyin qayta ko'rib chiqamiz.",
  "Boshni pastga tushirma — hali ko'p narsa oldinda.",
  "Xato qilish mumkin, lekin to'xtash mumkin emas.",
  "Nafas ol… va keyingi savolga o'taylik.",
  "Sen qila olasan. Bitta xato — oxiri emas.",
];
...
function openExamSettings() {
  ...
  if (!modal) {
    ...
    document.getElementById("examSettingsPause").onclick = () => {
      ...
      document.getElementById("examSettingsPause").textContent = examTimerPaused ? "Davom" : "Pause";
    };
  }
  applyExamFontScale();
...
function getSessionResultStats() {
  ...
  let correct = 0;
  let wrong = 0;
  Object.keys(examQuestionResults || {}).forEach((key) => {
    const r = examQuestionResults[key];
    if (!r) return;
    if (r.status === "correct") correct++;
    else wrong++;
  });
  // fallback if results map empty but score/mistakes exist
  if (correct === 0 && wrong === 0 && (score > 0 || mistakes.length > 0)) {
    correct = score || 0;
    wrong = mistakes.length || 0;
  }
  const solved = correct + wrong;
  const unsolved = Math.max(0, total - solved);
  const percent = solved === 0 ? 0 : Math.round((correct / solved) * 100);
  return { total, solved, correct, wrong, unsolved, percent, partial: unsolved > 0 };
}
...
function stopTest() {
  ...
  confirmBtn.onclick = function () {
    modal.hide();
    // Show professional result for partial attempt (e.g. 1 of 24)
    showResultPopup({ partial: true, fromExit: true });
  };
}
