...
function ensureExamHeaderStyles() {
  ...
  style.textContent = `
    ...
    body.exam-mode-active .mcq-question.exam-text-highlight,
    body.exam-mode-active .mcq-question ::selection {
      background: #fde047;
    }
    body.exam-mode-active.exam-highlight-mode .mcq-question {
      cursor: text;
    }
...
function createExamBottomBar() {
  ...
  bar.innerHTML = `
    ...
    <button type="button" class="exam-tool" id="exam-tool-strike" title="Variantni chizib tashlash">
      <i class="bi bi-slash-circle"></i><span>Chizish</span>
    </button>
    <button type="button" class="exam-tool" id="exam-tool-highlight" title="Belgilash / highlight">
      <i class="bi bi-highlighter"></i><span>Highlight</span>
    </button>
    <button type="button" class="exam-tool" id="exam-tool-pause" title="Taymerni to'xtatish / davom ettirish">
      <i class="bi bi-pause-circle"></i><span>Pause</span>
    </button>
    <button type="button" class="exam-tool" id="exam-tool-nav" title="Savollar navigatsiyasi">
      <i class="bi bi-grid-3x3-gap"></i><span>Savollar</span>
    </button>
    <button type="button" class="exam-tool danger-tool" id="exam-tool-stop" title="Testni to'xtatish">
  ...
  document.getElementById("exam-tool-strike").onclick = () => toggleExamStrikeMode();
  document.getElementById("exam-tool-highlight").onclick = () => toggleExamHighlightMode();
...
function hideExamBottomBar() {
  ...
  examStrikeMode = false;
  examHighlightMode = false;
  examTimerPaused = false;
  document.body.classList.remove("exam-strike-mode", "exam-highlight-mode");
}

function updateExamBottomBar() {
  const flagBtn = document.getElementById("exam-tool-flag");
  const strikeBtn = document.getElementById("exam-tool-strike");
  const highlightBtn = document.getElementById("exam-tool-highlight");
  ...
  if (strikeBtn) strikeBtn.classList.toggle("active", examStrikeMode);
  if (highlightBtn) highlightBtn.classList.toggle("active", examHighlightMode);
  if (pauseBtn) {
...
function toggleExamStrikeMode() {
  examStrikeMode = !examStrikeMode;
  if (examStrikeMode) examHighlightMode = false;
  document.body.classList.toggle("exam-strike-mode", examStrikeMode);
  document.body.classList.toggle("exam-highlight-mode", examHighlightMode);
  updateExamBottomBar();
}

function toggleExamHighlightMode() {
  examHighlightMode = !examHighlightMode;
  if (examHighlightMode) examStrikeMode = false;
  document.body.classList.toggle("exam-highlight-mode", examHighlightMode);
  document.body.classList.toggle("exam-strike-mode", examStrikeMode);
  updateExamBottomBar();
}
...
function openExamNavigator() {
  ...
  for (let i = 0; i < total; i++) {
    ...
    const bg = isCurrent ? "#1b3a3a" : flagged ? "#f59e0b" : "#e2e8f0";
    const color = isCurrent || flagged ? "#fff" : "#0f172a";
    html += `<button type="button" data-nav-q="${i}" style="padding:10px 0;border:none;border-radius:10px;font-weight:700;background:${bg};color:${color};cursor:pointer;">${i + 1}</button>`;
  }
  html += `</div><p style="margin:12px 0 0;font-size:12px;color:#64748b;text-align:center;">Sariq = flag · Qora = joriy</p>`;
...
function handleExamOptionHelperClick(btn, index) {
  if (examStrikeMode) {
    btn.classList.toggle("exam-struck");
    return true;
  }
  if (examHighlightMode) {
    btn.classList.toggle("exam-highlight");
    return true;
  }
  return false;
}
...
function showQuestion(isNavigatingBack = false) {
  ...
  q.a.forEach((ans, index) => {
    ...
    `;
    btn.onclick = () => {
      if (handleExamOptionHelperClick(btn, index)) return;
      selectAnswer(index);
    };
...
function selectAnswer(selectedIndex) {
  ...
  if (examStrikeMode || examHighlightMode) return;
...
window.toggleExamStrikeMode = toggleExamStrikeMode;
window.toggleExamHighlightMode = toggleExamHighlightMode;
