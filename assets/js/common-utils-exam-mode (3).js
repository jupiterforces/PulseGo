...
function ensureExamHeaderStyles() {
  ...
  style.textContent = `
    ...
    /* Test screen layout: header is sibling ABOVE content, not wrapping it */
    #test-screen {
      display: flex;
    ...
    /* Exam bar — normal in document flow; sticks only after scroll */
    ...
    /* Stronger bar once stuck to top while scrolling */
    ...
    .exam-bar .btn:active:not(:disabled) { transform: scale(0.97); }
    .exam-bar .btn:disabled {
      opacity: 0.4;
      cursor: not-allowed;
    }
    .exam-bar #exam-prev-btn {
      color: #475569;
      border-color: #cbd5e1;
      background: #fff;
    }
    .exam-bar #exam-prev-btn:hover:not(:disabled) {
      background: #f8fafc;
      border-color: #94a3b8;
      color: #0f172a;
    }
...
function bindExamBarSticky() {
  ...
  const onScroll = () => {
    // Stuck when bar has reached the top of the viewport
