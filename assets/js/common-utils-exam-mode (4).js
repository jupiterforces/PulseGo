...
function ensureExamHeaderStyles() {
  ...
  style.textContent = `
    ...
    /* Full-width UWorld-like top bar */
    #exam-bar.exam-bar,
    ...
    .exam-bar-center { flex: 1 1 auto; justify-content: center; min-width: 0; gap: 0.75rem; }
    .exam-bar-right { flex: 0 0 auto; margin-left: auto; }

    /* Progress like UWorld "4 / 4" */
    ...
    /* Icon-style controls on dark bar */
    .exam-bar .btn {
      display: inline-flex;
      align-items: center;
      justify-content: center;
    ...
    .exam-bar .btn:disabled {
      opacity: 0.35;
      cursor: not-allowed;
    }
    .exam-bar #exam-stop-btn {
      color: #fecaca;
    }
    ...
    /* UWorld-like question + options */
    body.exam-mode-active .mcq-question {
      font-size: 1.05rem;
      line-height: 1.65;
      color: #1e293b;
      font-weight: 400;
    ...
    body.exam-mode-active .mcq-options {
      display: flex;
      flex-direction: column;
      gap: 0;
    ...
    body.exam-mode-active .mcq-option:last-child { border-bottom: none; }
    body.exam-mode-active .mcq-option:hover:not(:disabled) {
      background: #eef5fa;
    }
    body.exam-mode-active .mcq-option.active {
      background: #e0f2fe;
    }
    body.exam-mode-active .mcq-option.correct {
      background: #dcfce7 !important;
    }
    body.exam-mode-active .mcq-option.wrong {
      background: #fee2e2 !important;
    }
    body.exam-mode-active .mcq-radio {
      flex-shrink: 0;
      width: 1.15rem;
      height: 1.15rem;
      margin-top: 0.15rem;
    ...
    body.exam-mode-active .mcq-option.active .mcq-radio {
      border-color: #0284c7;
      box-shadow: inset 0 0 0 3px #0284c7;
    }
    body.exam-mode-active .mcq-option.correct .mcq-radio {
      border-color: #16a34a;
      box-shadow: inset 0 0 0 3px #16a34a;
    }
    body.exam-mode-active .mcq-option.wrong .mcq-radio {
      border-color: #dc2626;
      box-shadow: inset 0 0 0 3px #dc2626;
    }
    body.exam-mode-active .mcq-option-letter {
      flex-shrink: 0;
      font-weight: 700;
      color: #334155;
      min-width: 1.4rem;
    }
    body.exam-mode-active .mcq-option-text {
      flex: 1;
      color: #1e293b;
    }
    body.exam-mode-active .mcq-controls {
      margin-top: 1.25rem;
      max-width: 42rem;
    }
    ...
    body.exam-mode-active .mcq-explanation {
      margin-top: 1rem;
      padding: 1rem 1.1rem;
      border-radius: 10px;
      background: #f1f5f9;
    ...
    body.exam-mode-active .mcq-explanation-title {
      font-weight: 700;
      margin-bottom: 0.4rem;
      color: #0f172a;
    }
    body.exam-mode-active .mcq-photo {
      margin: 0 0 1.25rem;
      max-width: 42rem;
    }
    body.exam-mode-active .mcq-photo img {
      max-width: 100%;
      border-radius: 12px;
    ...
    @media (max-width: 576px) {
      #exam-bar.exam-bar { padding: 0.5rem 0.65rem; min-height: 48px; gap: 0.35rem; }
      #test-title.exam-progress { font-size: 0.95rem; }
      #timer.tg-timer { font-size: 0.8rem; min-width: 3.8rem; padding: 0.3rem 0.55rem; }
      .exam-bar .exam-btn-label { display: none; }
      .exam-bar .btn { padding: 0.4rem 0.55rem; }
      #test-screen > #question-container { padding: 1.1rem 0.9rem 2rem; }
      body.exam-mode-active .mcq-question { font-size: 0.98rem; }
    }
  `;
  document.head.appendChild(style);
}
...
function bindExamBarSticky() {
  ...
  if (!bar || bar.dataset.stickyBound === "1") return;
  bar.dataset.stickyBound = "1";

  const onScroll = () => {
    const stuck = bar.getBoundingClientRect().top <= 1;
