...
function ensureExamHeaderStyles() {
  ...
  style.textContent = `
    ...
    .exam-header .exam-btn:active { transform: scale(0.97); }
    .exam-header .exam-btn:disabled {
      opacity: 0.45;
      cursor: not-allowed;
      transform: none;
    }
    ...
    .exam-btn-stop:hover { background: #b91c1c; }
    .exam-timer {
      display: inline-flex;
      align-items: center;
      gap: 6px;
    ...
    .exam-timer.warning { background: #7c2d12; border-color: #ea580c; color: #ffedd5; }
    .exam-timer.danger { background: #7f1d1d; border-color: #ef4444; color: #fee2e2; animation: exam-pulse 1s ease infinite; }
    ...
    body.exam-mode-active .navbar { display: none !important; }
    @media (max-width: 576px) {
      .exam-header { padding: 8px 10px; gap: 6px; }
      .exam-header .exam-btn { padding: 7px 10px; font-size: 12px; }
      .exam-timer { font-size: 14px; min-width: 64px; }
      .exam-progress { font-size: 12px; }
    }
  `;
  document.head.appendChild(style);
}

function createExamHeader() {
  ensureExamHeaderStyles();
  let header = document.getElementById("exam-header");
  ...
  header.innerHTML = `
    <div class="exam-header-left">
      <button type="button" class="exam-btn exam-btn-prev" id="exam-prev-btn" title="Oldingi savol">
        <i class="bi bi-chevron-left"></i> Oldingi
      </button>
      <button type="button" class="exam-btn exam-btn-stop" id="exam-stop-btn" title="Testni to'xtatish">
        <i class="bi bi-x-lg"></i> Stop
      </button>
    </div>
    <div class="exam-header-center">
      <span class="exam-progress" id="exam-progress-text">Savol 1 / 1</span>
      <span class="exam-timer" id="exam-timer">0:00</span>
    </div>
    <div class="exam-header-right">
      <button type="button" class="exam-btn exam-btn-next" id="exam-next-btn" title="Keyingi savol">
  ...
  document.getElementById("exam-prev-btn").onclick = () => previousQuestion();
  document.getElementById("exam-next-btn").onclick = () => nextQuestion();
  document.getElementById("exam-stop-btn").onclick = () => stopTest();

  return header;
}

function showExamHeader() {
  const header = createExamHeader();
...
function updateExamHeader() {
  ...
  if (progressEl) {
    progressEl.textContent = `Savol ${Math.min(currentIndex + 1, currentTest.length)} / ${currentTest.length}`;
  }
  ...
  if (nextBtn) {
    ...
    nextBtn.innerHTML = isLast && answerChecked
      ? `Natija <i class="bi bi-flag-fill"></i>`
      : `Keyingi <i class="bi bi-chevron-right"></i>`;
  }

  const legacyTimer = document.getElementById("timer");
  if (legacyTimer) legacyTimer.innerText = formatTime(timeLeft);
